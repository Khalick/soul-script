          ...
          {selectedMood && (
            <div className="dashboard-card" style={{ animationDelay: '0.7s' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                {quickTags.map((tag) => (
                  <button key={tag} onClick={() => handleTagToggle(tag)} style={{ padding: '12px 24px', borderRadius: '25px', fontSize: '15px', fontWeight: '600', border: 'none', cursor: 'pointer', transition: 'all 0.3s', background: selectedTags.includes(tag) ? 'linear-gradient(135deg, #667eea, #764ba2)' : 'rgba(255, 255, 255, 0.15)', color: 'white', boxShadow: selectedTags.includes(tag) ? '0 8px 20px rgba(118, 75, 162, 0.4)' : 'none', transform: selectedTags.includes(tag) ? 'scale(1.05)' : 'scale(1)' }}>
                    {tag}
                  </button>
                ))}
              </div>
